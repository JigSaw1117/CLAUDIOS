"""
Generador de las interfaces actualizadas para CASO_2 y TALLER 1 con:
1. Logo oficial SVG de la Universidad Andina del Cusco (UAC).
2. Tamaños de letra grandes y de alto contraste (sin textos diminutos de 9px).
3. Estilizado limpio, profesional y académico sin clichés de IA.
4. Conservación de la paleta oficial de colores UAC y precisión matemática.
"""

import os

# Logo SVG oficial estilizado de la Universidad Andina del Cusco (UAC)
UAC_LOGO_SVG = r'''<svg class="uac-logo-img" viewBox="0 0 240 60" xmlns="http://www.w3.org/2000/svg" style="height:44px; width:auto;">
  <!-- Emblem background -->
...
def get_wine_html(is_root_caso=False):
    nav_link_prefix = "../" if is_root_caso else "../"
...
<title>Wine Quality Predictor — Universidad Andina del Cusco | Caso 2</title>
<meta name="description" content="Predicción de calidad de vino mediante Regresión Lineal Múltiple y Regresión Polinómica. Universidad Andina del Cusco."/>
...
:root {{
  ...
  --uac-border: rgba(0,180,216,0.25);
...
<header class="uac-navbar">
  <div class="uac-nav-left">
    <a href="{nav_link_prefix}index.html" style="text-decoration:none; display:flex; align-items:center; gap:10px;">
      {UAC_LOGO_SVG}
    </a>
  </div>
  <div class="uac-nav-links">
    <a href="{nav_link_prefix}index.html">🏠 Portada Principal</a>
    <a href="{nav_link_prefix}CASO_1/index.html">📌 Caso 1</a>
    <a href="{nav_link_prefix}CASO_2/index.html" class="active">🍷 Caso 2 (Vinos)</a>
    <a href="{nav_link_prefix}CASO_3/index.html">📈 Caso 3</a>
  </div>
</header>

<!-- HEADER DE LA PÁGINA -->
<header class="page-header">
  <div class="header-badge">UNIVERSIDAD ANDINA DEL CUSCO — INTELIGENCIA ARTIFICIAL</div>
  <h1>Predictor de Calidad del Vino Tinto</h1>
  <p>Comparación en tiempo real entre el modelo de <strong>Regresión Lineal Múltiple (OLS)</strong> y <strong>Regresión Polinómica (Grado 2)</strong> entrenados con 1,143 muestras del dataset <code>WineQT.csv</code>.</p>
  
  <div class="mode-selector">
    <button class="mode-btn lin active" id="btnModeLin" onclick="setMode('linear')">📈 Modelo Lineal (OLS)</button>
    <button class="mode-btn both" id="btnModeBoth" onclick="setMode('both')">⚖️ Comparar Ambos Modelos</button>
    <button class="mode-btn poly" id="btnModePoly" onclick="setMode('poly')">📐 Modelo Polinómico (Grado 2)</button>
  </div>
</header>
...
<main class="container">
  <div class="main-grid">
    ...
    <div>
      <div class="card">
        <div class="card-title" id="formTitle">🧪 Propiedades Físico-Químicas del Vino</div>
        <form id="wineForm">
          <div class="form-grid" id="formGrid"></div>
          <button type="submit" class="predict-btn lin-btn" id="btnPredict">🔬 Calcular Calidad Predicha</button>
        </form>
      </div>

      <!-- DESCRIPCIÓN DE VARIABLES -->
      <div class="card">
        <div class="card-title">📚 Guía de Variables Físico-Químicas</div>
        <div class="info-chips-grid" id="infoGrid"></div>
      </div>

      <!-- FÓRMULA PASO A PASO -->
      <div class="card hidden" id="formulaCard">
        <div class="card-title" id="formulaTitle">🧮 Desglose Matemático de la Fórmula</div>
        <div class="formula-box" id="formulaBox"></div>
        <div class="terms-list" id="termsList"></div>
      </div>
    </div>

    <!-- COLUMNA DERECHA: RESULTADOS Y MÉTRICAS -->
    <div>
      <!-- RESULTADOS PREDICCIÓN -->
      <div class="card">
        <div class="card-title">📊 Resultado de Predicción (Escala 0 a 10)</div>
        <div class="results-container" id="resultsContainer">
          
          <!-- RESULTADO LINEAL -->
          <div class="result-card lin-res" id="resLinCard">
            <span class="res-tag lin">📈 Regresión Lineal</span>
            <div id="resLinEmpty" style="color:var(--text-muted); font-size:14px;">
              Ingresa los parámetros y presiona <strong>Calcular Calidad</strong>
            </div>
            <div id="resLinContent" class="hidden">
              <div class="score-display lin-score" id="scoreLin">0.00</div>
              <div class="quality-label" id="labelLin">—</div>
              <div class="res-meta" id="metaLin"></div>
            </div>
          </div>

          <!-- RESULTADO POLINÓMICO -->
          <div class="result-card poly-res hidden" id="resPolyCard">
            <span class="res-tag poly">📐 Regresión Polinómica</span>
            <div id="resPolyEmpty" style="color:var(--text-muted); font-size:14px;">
              Ingresa los parámetros y presiona <strong>Calcular Calidad</strong>
            </div>
            <div id="resPolyContent" class="hidden">
              <div class="score-display poly-score" id="scorePoly">0.00</div>
              <div class="quality-label" id="labelPoly">—</div>
              <div class="res-meta" id="metaPoly"></div>
            </div>
          </div>
      ...
      <div class="card hidden" id="cmpCard">
        <div class="card-title">⚖️ Comparación Directa de Predicciones</div>
        <table class="cmp-table">
          <thead>
            <tr>
              <th>Métrica</th>
              <th style="color:var(--lin-color)">📈 Lineal</th>
              <th style="color:var(--poly-color)">📐 Polinómico</th>
              <th>Mejor Opción</th>
            </tr>
          </thead>
      ...
      <div class="card">
        <div class="card-title">📊 Métricas del Modelo en Dataset WineQT</div>
        <div class="stats-grid">
          <div class="stat-box">
            <div class="val lin-v">0.3171</div>
            <div class="lbl">R² Test Lineal</div>
          </div>
          <div class="stat-box">
            <div class="val poly-v">0.2809</div>
            <div class="lbl">R² Test Poly</div>
          </div>
          <div class="stat-box">
            <div class="val lin-v">0.6165</div>
            <div class="lbl">RMSE Lineal (pts)</div>
          </div>
          <div class="stat-box">
            <div class="val poly-v">0.6326</div>
            <div class="lbl">RMSE Poly (pts)</div>
          </div>
        </div>
        <div class="warning-box">
          <strong>⚠️ Análisis Estadístico de Sobreajuste (Overfitting):</strong><br>
          El modelo polinómico logra R² = 0.4591 en entrenamiento, pero su rendimiento cae a R² = 0.2809 en prueba. Al expandir a 77 variables sobre 914 muestras, memoriza el ruido. El modelo lineal (R² = 0.3171) es más robusto y fiable.
        </div>
      </div>

      <!-- COEFICIENTES BARS -->
      <div class="card">
        <div class="card-title" id="coefTitle">📈 Coeficientes Beta (Estandarizados)</div>
...
<script>
/**
 * PARÁMETROS MODELO 1: REGRESIÓN LINEAL MÚLTIPLE (OLS)
 */
const LIN = {{
...
/**
 * PARÁMETROS MODELO 2: REGRESIÓN POLINÓMICA (GRADO 2)
 */
const POLY = {{
...
function showRes(y, T, side) {{
  ...
  const empId = isL ? "resLinEmpty" : "resPolyEmpty";
  const cntId = isL ? "resLinContent" : "resPolyContent";
  const scId  = isL ? "scoreLin" : "scorePoly";
  const lbId  = isL ? "labelLin" : "labelPoly";
  const mtId  = isL ? "metaLin" : "metaPoly";
  
  document.getElementById(empId).classList.add("hidden");
...
function buildCmp(ly, py) {{
  ...
  const lc = Math.min(10, Math.max(0, ly));
  const pc = Math.min(10, Math.max(0, py));
...
function showFormula(T, m) {{
  ...
  const fti = document.getElementById("formulaTitle");
...
function setMode(m) {{
  ...
  const pb = document.getElementById("btnPredict");
  const resLinCard = document.getElementById("resLinCard");
  const resPolyCard = document.getElementById("resPolyCard");
  ...
  if (m === 'linear') {{
    ...
    resPolyCard.classList.add("hidden");
    ...
    resultsContainer.className = "results-container";
    resLinCard.classList.add("hidden");
...
# Generar archivos
with open(r"c:\Users\Usuario\Desktop\OPENCODE\TALLER 1\wine_predictor_v2.html", "w", encoding="utf-8") as f:
    f.write(get_wine_html(is_root_caso=False))
...
print("Archivos de la interfaz de vinos actualizados exitosamente.")
